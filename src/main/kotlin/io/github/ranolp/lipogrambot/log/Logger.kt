...
import io.github.ranolp.lipogrambot.general.options.CoreOption
...
object Logger {
    ...
    val logLevel: LogLevel by lazy {
        LogLevel.values().first { it.name == CoreOption.logLevel.toUpperCase() }
    }
    ...
    private fun log(level: LogLevel, text: String) {
        ...
        if (formatter === null) {
            formatter = DateTimeFormatter.ofPattern(CoreOption.dateSyntax)
        }
